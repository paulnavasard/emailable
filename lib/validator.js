...
proto._validateAddress = function() {
  var regex = /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i;
  var valid = regex.exec(this.email) !== null;
  valid ? this._nextAction() : this._error('Invalid Email Address');
};

proto._parseEmail = function() {
  var pattern = /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i,
  match = pattern.exec(this.email);
  this.segments = match == void 0 ? undefined : {
    email: match[0],
    user: match[1],
    host: match[2]
  };
...
proto._onData = function(chunk) {
  ...
  this._nextAction(line);
};

proto._nextAction = function() {
...
proto._parseGreeting = function(line) {
  ...
  this.sendCommand('HELO katalus.com');
};
...
proto._mailFrom = function(line) {
  ...
  this.sendCommand('MAIL FROM:<email@katalus.com>');
};
...
proto._finish = function(line) {
  ...
  this._callBackMessage(valid, line);

  this.sendCommand('QUIT');
};

proto._error = function(error) {
  var message = typeof error == 'string' ? error : error.message;
  this._callBackMessage(false, message);
};

proto._callBackMessage = function(valid, message) {
  this._cb && this._cb(undefined, { valid: valid, message: message });
};
